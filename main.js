...
function fillAndSearch() {
    ...
    if (searchtj.value !== searchCourseName) {
        searchtj.value = searchCourseName;
    }
...
function find() {
    ...
    if (iframe_document.getElementById('queryButton').textContent.search('正在') !== -1) {
        setTimeout(find, checkSearchStatusInterval);
        return;
    }
    ...
    setTimeout(() => {
        ...
        if (foundFlag) {
            ...
            fillAndSearch();
            setTimeout(find, delayBeforeRetry);
        }
    }, timeBeforeCheckAfterSearchFound);
}
